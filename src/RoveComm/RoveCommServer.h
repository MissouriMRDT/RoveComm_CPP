 ...
 * Each server listens to one port on its own network thread.
 * Yes I'm a Java user, how could you tell?
 *
 * @author OcelotEmpire (hobbz.pi@gmail.com)
 * @date 2023-11-13
 ******************************************************************************/
...
{
    public:
        virtual ~RoveCommServer();    // POLYMORPHISM GO BRRRRRR

        // Create and close network thread
        virtual void init();
        virtual void shutdown();

        // Write a RoveCommPacket to an address
        virtual void write(RoveCommPacket& cPacket) const;
        virtual RoveCommPacket& read() const;
        // Sets the callback function for any incoming packets with the given data id
        virtual void setCallback(RoveCommDataId unId, RoveCommCallback fCallback);
...
/******************************************************************************
 * @brief Handle lifetimes of RoveCommServer's and defer work to them
 *
 *
 * @author OcelotEmpire (hobbz.pi@gmail.com)
 * @date 2023-11-14
...
{
    public:
        static void createServerOnPort(unsigned int port, RoveCommServerProtocol protocol = RoveCommServerProtocol::UDP);
